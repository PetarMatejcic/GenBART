...
private:
    ...
    double predict_tree_row(const double* x, int64_t root) const;

    inline bool is_leaf(int64_t node) const noexcept {
        return left[node] == -1;
    }

    inline int64_t root_of_tree(int64_t tree_id) const noexcept {
        return tree_offset[tree_id];
    }

    inline double predict_tree_row_ptr(const double* x, int64_t root) const noexcept {
        int64_t node = root;
        while (!is_leaf(node)) {
            node = (x[variable[node]] <= value[node])
                ? left[node]
                : right[node];
        }
